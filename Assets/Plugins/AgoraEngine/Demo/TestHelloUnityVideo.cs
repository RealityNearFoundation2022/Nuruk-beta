...
public class TestHelloUnityVideo
{
    ...
    public void joinAudience(string channel)
    {
        ...
        mRtcEngine.OnWarning = (int warn, string msg) =>
        {
            Debug.LogWarningFormat(
                "Warning code:{0} msg:{1}",
                warn,
                IRtcEngine.GetErrorDescription(warn)
            );
        };
        ...
        else
        {
            ...
            mRtcEngine.JoinChannel(
                ...
                options: new ChannelMediaOptions(
                    AudioVideoState.subAudio,
                    AudioVideoState.subVideo,
                    AudioVideoState.pubAudio,
                    AudioVideoState.pubVideo
                )
            );
        }
    ...
    private void SetButtons()
    {
        ...
        {
            MuteAudioButton.Setup(
                initOnOff: false,
                onStateText: "Mute Local Audio",
                offStateText: "Unmute Local Audio",
                callOnAction: () =>
                {
                    mRtcEngine.MuteLocalAudioStream(true);
                },
        ...
        {
            MuteVideoButton.Setup(
                initOnOff: false,
                onStateText: "Mute Local video",
                offStateText: "Unmute Local video",
                callOnAction: () =>
                {
        ...
        if (ChannelButton != null)
        {
            ChannelButton.Setup(
                initOnOff: false,
                onStateText: mChannelName + "2",
                offStateText: mChannelName,
                callOnAction: () =>
                {
        ...
        ChannelButton.GetComponent<Button>().interactable =
            ClientRole == CLIENT_ROLE_TYPE.CLIENT_ROLE_AUDIENCE;
    }
    ...
    void OnClientRoleChangeFailedHandler(
    ...
    {
        Debug.Log(
            "OnVolumeIndicationHandler speakerNumber:"
                + speakerNumber
                + " totalvolume:"
                + totalVolume
        );
    ...
    {
        Debug.LogFormat(
            "OnLeaveChannelSuccess ---- duration = {0} txVideoBytes:{1} ",
            stats.duration,
            stats.txVideoBytes
        );
    ...
    void OnVideoSizeChangedHandler(uint uid, int width, int height, int rotation)
    {
        Debug.LogWarning(
            string.Format(
                "OnVideoSizeChangedHandler, uid:{0}, width:{1}, height:{2}, rotation:{3}",
                uid,
                width,
                height,
                rotation
